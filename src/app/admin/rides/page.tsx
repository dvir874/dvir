...
/* The board moved here from /couple/<token>/rides.
 *
 * Introducing two guests to each other is a judgement about people Dvir knows
 * and the couple are living through — he asked for it to be his call, not a
 * button a couple presses at midnight on somebody they half remember. The
 * couple keep the wedding; the matchmaking is his.
 *
 * Same component, same consent messages, same canonicalisation. The only
 * difference is which query fills it: an event id instead of a couple token. */
function RidesBoard() {
    ...
    Promise.all([
      ...
      .then(([g, evs]) => {
        ...
        if (Array.isArray(evs)) {
          ...
          if (ev?.name) setEventName(ev.name as string);
        }
        setLoading(false);
      })
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100dvh", background: C.ivory, fontFamily: "'Heebo', sans-serif", paddingBottom: 120 }}>
      ...
      <div style={{ background: "#fff", borderBottom: `1px solid ${C.border}`, padding: "16px 20px", display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 10 }}>
        <a href="/admin" style={{ color: C.dark, minWidth: 44, minHeight: 44, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <ArrowRight size={22} />
        </a>
        <h1 style={{ fontFamily: "'Frank Ruhl Libre', serif", fontSize: 20, fontWeight: 700, color: C.dark, margin: 0 }}>
          🚗 לוח טרמפים{eventName ? ` — ${eventName}` : ""}
        </h1>
      </div>
...
/* useSearchParams suspends, and a page that reads it without a boundary fails
   the production prerender rather than the dev server — which is why this was
   only visible at build time. */
