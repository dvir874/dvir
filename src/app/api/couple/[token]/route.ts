...
function computeForecastServer(guests: { status: string; guest_count: number }[]): Forecast {
  ...
  const confirmRate = responders > 0 ? Math.round((confirmed.length / responders) * 100) : 50;
  const responseRate = total > 0 ? Math.round((responders / total) * 100) : 0;
  const pendingConfirm = pending * (confirmRate / 100);
  return {
    confirmedAttendees, responseRate, confirmRate, pendingGuests: pending,
...
function computeHealthServer(
  guests: { status: string; opened_at: string | null; response_time: string | null }[],
  eventDate: string
): HealthScore {
  const now      = Date.now();
  const total    = guests.length;
  ...
  return {
    score, tier,
    label: { green: 'האירוע במצב מצוין.', yellow: 'יש מקום לשיפור — שווה לשלוח תזכורות.', red: 'מספר רב של מוזמנים עדיין לא ענו.' }[tier],
    breakdown: [
...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  const [guestsRes, budgetRes, seatingTablesRes, assignmentsRes, tasksRes, giftsRes] = await Promise.all([
    supabase.from('guests').select('id, status, guest_count, opened_at, response_time').eq('event_id', event.id),
    ...
    supabase.from('seating_assignments').select('id').eq('event_id', event.id),
    supabase.from('wedding_tasks').select('id, completed').eq('event_id', event.id),
    supabase.from('gifts').select('amount').eq('event_id', event.id),
  ]);
  ...
  // Seating
  const tables       = seatingTablesRes.data ?? [];
  const totalSeats   = tables.reduce((s, t) => s + (t.capacity ?? 0), 0);
  const assignedSeats = (assignmentsRes.data ?? []).length;
  ...
  // Gifts
  const giftItems  = giftsRes.data ?? [];
  const giftsTotal = giftItems.reduce((s, g) => s + (g.amount ?? 0), 0);

  return NextResponse.json({
    event,
    stats:   { total, confirmed, declined, pending, attendees, responseRate },
    forecast: total > 0 ? computeForecastServer(guests) : null,
    health:   total > 0 ? computeHealthServer(guests, event.date) : null,
    budget:  { planned: budgetPlanned, actual: budgetActual, remaining: budgetPlanned - budgetActual, itemCount: budgetItems.length },
    seating: { totalSeats, assignedSeats, tableCount: tables.length },
    tasks:   { total: allTasks.length, completed: completedTasks },
    gifts:   { total: giftsTotal, count: giftItems.length },
  });
}
