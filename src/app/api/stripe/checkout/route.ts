...
export async function POST(req: NextRequest) {
  ...
  // event_id is optional — lead-stage deposits (quote page) have no event yet
  if (!amount || amount <= 0) {
  ...
  let event: { name: string; client_email: string | null; client_phone: string | null } | null = null;
  if (event_id) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { event_id: event_id ?? "", amount: String(amount) },
    success_url: `${origin}/payment/success?session_id={CHECKOUT_SESSION_ID}${event_id ? `&event_id=${event_id}` : ""}`,
