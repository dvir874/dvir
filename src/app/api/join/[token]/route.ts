...
const JoinSchema = z.object({
  ...
  attending: z.enum(["yes", "no", "chuppah"]),
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (existing) {
    let { error } = await sb
      .from("guests")
      .update({ name: name.trim(), status, guest_count: coming ? guest_count : 0, response_time: new Date().toISOString(), ...groupPatch })
      .eq("id", existing.id);
    if (error && Object.keys(groupPatch).length) {
      // graceful pre-migration fallback (source_group column may not exist yet)
      ({ error } = await sb
        .from("guests")
        .update({ name: name.trim(), status, guest_count: coming ? guest_count : 0, response_time: new Date().toISOString() })
        .eq("id", existing.id));
    }
  ...
  const baseRow = {
    ...
    phone: cleanPhone,
    status,
    guest_count: coming ? guest_count : 0,
