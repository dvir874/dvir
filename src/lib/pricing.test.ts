...
test("a record is the unit, not a guest", () => {
  ...
  assert.equal(quoteFor(400, "basic").total, 500);
...
test("the rides group is priced on top and has its own floor", () => {
  ...
  assert.deepEqual(q.lines.map(l => l.amount), [500, 200]);
  assert.equal(q.total, 700);
...
test("nonsense input does not produce a nonsense price", () => {
  ...
  assert.equal(PER_RECORD_BASIC * 400, 500);
});
...
test("a guest count is converted to records before it is priced", () => {
  ...
  assert.equal(quoteFor(records, "basic", false).total, MIN_CHARGE_BASIC);
});
...
test("the floor covers every wedding until about six hundred people", () => {
  ...
  for (const people of [150, 200, 300, 430, 500]) {
    assert.equal(quoteFor(recordsFromGuests(people), "basic", false).total, MIN_CHARGE_BASIC,
      `${people} people should still be at the floor`);
  }
  assert.ok(quoteFor(recordsFromGuests(800), "basic", false).total > MIN_CHARGE_BASIC);
});

test("a quote still clears its own message cost", () => {
  /* The floor is a floor, not a loss-leader — it has to beat what Meta charges. */
